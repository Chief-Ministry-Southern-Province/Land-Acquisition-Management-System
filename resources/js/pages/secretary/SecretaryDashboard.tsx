...
import { getPendingApprovals } from '@/services/secApprovalService';
...
export default function SecretaryDashboard() {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      console.error('Failed to fetch Secretary dashboard data:', err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const pendingProjects = projects.filter((p) => p.sec_status === 'pending');
  const approvedProjects = projects.filter((p) => p.sec_status === 'approved');
  const rejectedProjects = projects.filter((p) => p.sec_status === 'rejected');
    ...
    <div className="space-y-6">
      ...
      <div className="bg-primary/5 border-primary/10 rounded-xl border p-6">
        <h1 className="text-primary text-2xl font-bold">
          {t('welcome_back_sec', 'Welcome Back, ')} {username}
        </h1>
        <p className="text-muted-foreground mt-1 text-sm">
          {t(
            'sec_dashboard_subtitle',
            'Secretary Dashboard • Southern Province Land Acquisition Management System',
          )}
        </p>
      </div>
      ...
      <div className="space-y-3">
        ...
        {loading ? (
          ...
          <div className="bg-card border-border text-muted-foreground flex h-48 flex-col items-center justify-center rounded-lg border text-sm">
            ...
            {t(
              'no_pending_sec_action',
              'No pending submissions awaiting SEC action.',
            )}
          </div>
        ) : (
          <DataTable

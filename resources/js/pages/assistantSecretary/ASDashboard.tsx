...
import { getPendingApprovals } from '@/services/asApprovalService';
...
export default function ASDashboard() {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      console.error('Failed to fetch AS dashboard data:', err);
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-primary/5 border-primary/10 rounded-xl border p-6">
        <h1 className="text-primary text-2xl font-bold">
          {t('welcome_back_as', 'Welcome Back, ')} {username}
        </h1>
        <p className="text-muted-foreground mt-1 text-sm">
          {t(
            'as_dashboard_subtitle',
            'Assistant Secretary Dashboard • Southern Province Land Acquisition Management System',
          )}
        </p>
      </div>
      ...
      <div className="space-y-3">
        ...
        {loading ? (
          ...
          <div className="bg-card border-border text-muted-foreground flex h-48 flex-col items-center justify-center rounded-lg border text-sm">
            ...
            No pending submissions awaiting AS action.
          </div>
        ) : (
          <DataTable
...
ASDashboard.layout = (page: React.ReactNode) => (
